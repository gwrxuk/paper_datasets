...
csv_file = 'langid/video_description_language.csv'
html_file = 'langid/description.html'
...
max_lang_count = max(row['lang_count'] for row in desc_rows)
min_lang_count = min(row['lang_count'] for row in desc_rows)
max_lang_videos = [row for row in desc_rows if row['lang_count'] == max_lang_count]
min_lang_videos = [row for row in desc_rows if row['lang_count'] == min_lang_count]
...
with open(html_file, 'w', encoding='utf-8') as f:
    f.write('<html><head><title>LangID Description Language Summary</title></head><body>')
    f.write('<h1>LangID Description Language Summary</h1>')
    f.write(f'<p><strong>Number of videos:</strong> {num_videos}</p>')
    # List all languages in the unique languages section
    all_langs_str = ', '.join(all_columns)
    f.write(f'<p><strong>Number of unique languages in description:</strong> {num_languages} ({all_langs_str})</p>')
    ...
    f.write(f'<p><strong>Most languages used in a description:</strong> {max_lang_count}</p>')
    f.write('<ul>')
    for row in max_lang_videos:
        langs_in_desc = [lang for lang in all_columns if row['langs'].get(lang, 0) > 0]
        langs_str = ', '.join(langs_in_desc)
        f.write(f'<li>{row["youtube_id"]}: <br><em>Languages: {langs_str}</em></li>')
    f.write('</ul>')
    f.write(f'<p><strong>Least languages used in a description:</strong> {min_lang_count}</p>')
    f.write('<ul>')
